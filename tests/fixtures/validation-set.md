...
A small held-out fixture for measuring drift in our LLM annotator's behavior over time. The intent is to catch regressions when the system prompt changes, when Claude Haiku 4.5 gets a snapshot bump, when we change tool-schema details, or when we add/modify codes in either codebook (Moore primary, ismyaialive `iaa-` supplemental).
