...
The other patterns in `js/matchers.js` (P1–P10) are kept as:
- **Fallback annotations** if the API is over-budget, rate-limited, or unreachable.
- **Reference patterns** that informed the system prompt's examples.

They are not the primary surface anymore.

### Main pass — LLM with Moore et al. codebook

The main analysis is a single Claude Haiku 4.5 call with the system prompt at `docs/system-prompt.md`, which embeds the validated 28-code Moore et al. (2026) codebook verbatim. Prompt caching reduces per-call cost to roughly $0.005–$0.02.
